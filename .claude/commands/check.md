...
**MANDATORY WORKFLOW:**
```
1. Create safety checkpoint → /commit "checkpoint(pre-check): Save state before fixing issues"
2. Run checks → Find issues
3. IMMEDIATELY spawn agents to fix ALL issues
4. Re-run checks → Find remaining issues
5. Fix those too
6. REPEAT until EVERYTHING passes
7. Commit fixes → /commit "fix(quality): Resolve all linting, test, and build issues"
```
...
**BEFORE STARTING**:
```bash
/commit with message: "checkpoint(pre-check): Save state before fixing code quality issues"
```

**AFTER SUCCESS**:
```bash
/commit with message: "fix(quality): Resolve all linting, test, and build issues - all checks passing"
```
